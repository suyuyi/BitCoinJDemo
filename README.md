...
### 部分闪退问题的改进
> 在部分情况下钱包可能会出现闪退的情况，这大多数时候是由于钱包的主模块single/multi_v2中钱包的运行分为两部分界面输入输出处理部分和同步收发部分，后者在一个线程中运行，在使用钱包时如果按返回键退出，很可能会造成线程中的相关文件（如文件锁）无法被正确释放，使得之后再进入其他钱包界面时出现问题。  
> 现在给出的解决方向较为简单粗暴：将single、multi_v2中的exit函数设置为直接退出，然后清空后台程序再次进入即可使用。后续改进可能需要从工具库的底层实现代码方面入手，对其中的部分类进行重用。
